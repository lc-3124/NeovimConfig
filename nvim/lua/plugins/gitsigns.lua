...
return {
  ...
  opts = {
    ...
    on_attach = function(bufnr)
      ...
      map("n", "<leader>gs", gs.stage_hunk, { desc = "暂存当前改动" })
      map("v", "<leader>gs", function() gs.stage_hunk({ vim.fn.line("."), vim.fn.line("v") }) end, { desc = "暂存选中改动" })
      map("n", "<leader>gr", gs.reset_hunk, { desc = "撤销当前改动" })          -- 放弃当前改动
      map("n", "<leader>gS", gs.stage_buffer, { desc = "暂存整个文件" })     -- 全部暂存
      map("n", "<leader>gu", gs.undo_stage_hunk, { desc = "撤销暂存" })      -- 取消暂存
      map("n", "<leader>gR", gs.reset_buffer, { desc = "重置整个文件" })         -- 放弃整个文件
      map("n", "<leader>gp", gs.preview_hunk, { desc = "预览改动" })         -- 浮窗预览
      map("n", "<leader>gb", function() gs.blame_line({ full = true }) end, { desc = "查看当前行归属" }) -- 这一行最后是谁改的
      map("n", "<leader>gB", function() gs.blame() end, { desc = "查看整个文件归属" }) -- 逐行显示每行作者
      ...
      map("n", "<leader>gT", gs.toggle_current_line_blame, { desc = "切换行内归属显示" }) -- 行尾常驻显示当前行作者
    end,
  },
}
